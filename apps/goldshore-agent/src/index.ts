...
async function handleRequest(request: Request, env: Env, ctx: ExecutionContext, user: AccessUser): Promise<Response> {
  ...
  if (!user.groups.includes('system')) {
      return new Response('Forbidden: You are not a system user', { status: 403 });
  }
...
export default {
    ...
    const user = await verifyCfAccessJwt(request, env.CF_TEAM_DOMAIN, env.CF_ACCESS_AUD);
